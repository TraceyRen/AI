...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        for each in self.keys():
            self[each] = float(self[each])/totalValue
    ...
    def sample(self):
        ...
        for item in self.items():
            distri.append(item[0])
            w += float(item[1]) / float(self.total())
            weight.append(w)

        randomNum = random.random()
        for each, value in enumerate(distri):
            if randomNum <= weight[each]:
                return value
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # B(ghostPos) = p(obs|ghostPos) * B'(ghostPos)
        for ghost in self.allPositions:
            prob = self.getObservationProb(observation, pacmanPosition, ghost, jailPosition)
            self.beliefs[ghost] *= prob
    ...
    def elapseTime(self, gameState):
        ...
        # B(newPos, oldPos) = p(newPos|oldPos) * B(oldPos)
        empty = DiscreteDistribution()
        for oldPos in self.beliefs.keys():
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos in newPosDist.keys():
                empty[newPos] += newPosDist[newPos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        eachNum = int(self.numParticles / len(self.legalPositions))
        for particle in self.legalPositions:
            for _ in range(eachNum):
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        ghostsPositions = self.legalPositions
        ghostAgent = self.numGhosts
        particles = list(itertools.product(ghostsPositions, repeat=ghostAgent))
        random.shuffle(particles)

        copyTimes = int(self.numParticles/len(particles))
        remainder = (self.numParticles)%len(particles)
        for _ in range(copyTimes):
            for each in particles:
                self.particles.append(each)
        for i in range(remainder):
            self.particles.append(particles[i])
